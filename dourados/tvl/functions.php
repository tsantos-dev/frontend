...
function prefix_filter_news_permalink( $url, $post ) {
    ...
    $link_externo = get_post_meta( $post->ID,'link_externo',  true );
...
function pdf_init(){
    add_meta_box('my-pdf', __('Documento PDF','tvl_theme'), 'pdf_link', array('post', 'leis', 'page'), 'normal', 'low');
    }
function pdf_link(){
    global $post;
    ...
    $query_pdf_args = array(
        ...
        // 'post_mime_type' => array('aplicativo/doc','aplicativo/pdf')
        'post_status' => 'inherit',
        'posts_per_page' => -1,
        );
